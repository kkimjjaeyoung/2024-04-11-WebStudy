	...
	public void genieDate() {
		...
		try {
			...
			for(int i=1; i<=4; i++) {
				...
				for(int j=0; i<title.size(); j++) {
					System.out.println(k);
					System.out.println(title.get(j).text());						//<a>태그 안의 텍스트값 가져오기
					System.out.println(singer.get(j).text());
					System.out.println(album.get(j).text());
					System.out.println(poster.get(j).attr("src"));					//attr : src의 속성값 가져오기
