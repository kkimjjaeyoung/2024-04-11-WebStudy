...
<style type="text/css">
	.brow{margin: 0px auto; width: 800px;}
</style>
</head>
<body>
		...
		<div class="brow">
			<h2 class="sectiontitle">자유 게시판</h2>
				<table class="table">
					<tr>
						<td>
						<a href='#' class="btn btn-sm btb-primary">새 글</a>
						</td>
					</tr>
				</table>
				<table class="table">
					...
					<c:forEach var="vo" items="${list }">	<!-- for(BoardVO vo:list) -->
						<tr>
							<td width="10%" class="text-center">${vo.no }</td>
							<td width="10%" >${vo.setsubject() }
							&nbsp; <c:if test="${today==vo.dbday }"><sup><img src="../board/new.gif"></sup></c:if>
							</td>
							<td width="10%" class="text-center">${vo.name }</td>
							<td width="10%" class="text-center">${vo.getDbday() }</td>
							<td width="10%" class="text-center">${vo.hit }</td>
						</tr>
						<c:set vat="count" value="${count-1 }"/>
					</c:forEach>
				</table> 
				<table class="table">
					<tr>
						<td class="text-left inline">
							...
							<input tpye="text" name="ss" class="input-sm" size=15>
