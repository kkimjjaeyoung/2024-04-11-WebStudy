...
public class BoardDAO {
   ...
   public boolean boardUpdate(BoardVO vo) {
	   ...
	   try {
		   ...
		   System.out.println("boardUpdate오류");
		   ex.printStackTrace();
	}finally {
		dbConn.disConnection(conn, ps);
	}
   ...
   public boolean boardDelete(int no, String pwd) {
	   ...
	   try {
		   ...
		   System.out.println("boardDelete오류");
		   ex.printStackTrace();
	}finally {
		dbConn.disConnection(conn, ps);
	}
   ...
   // 6. 검색 => <select>
   public List<BoardVO> listBoardFind(int page){
	   List<BoardVO> list=new ArrayList<BoardVO>();
	   try {
		   conn=dbConn.getConnection();
		   String spl="SELECT name, subject, content FROM ( "
		   ...
		   System.out.println("boardfind오류");
		   ex.printStackTrace();
	}finally {
		dbConn.disConnection(conn, ps);
	}
